...
import { completeRefund, type RefundActor } from "@/lib/refund";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const providerRefundId = cleanText(
      payload.providerRefundId ??
        payload.ProviderRefundId,
    );

    const manualTransactionId = cleanText(
      payload.manualTransactionId ??
        payload.ManualTransactionId ??
    ...
    if (!providerRefundId && !manualTransactionId) {
      return NextResponse.json(
        {
          ok: false,
          error: {
            code: "INVALID_ARGUMENT",
            message:
              "providerRefundId or manualTransactionId is required",
          },
        },
        { status: 400 },
      );
    ...
    const result = await completeRefund({
      ...
      manualTransactionId: manualTransactionId || undefined,
    ...
    console.error("admin refund complete POST error", error);
