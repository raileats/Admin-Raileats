    ...
    >
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div ref={stationBoxRef} className="relative">
            ...
            {stationOpen && (
              <div className="absolute z-30 mt-1 max-h-64 w-full overflow-auto rounded-md border border-slate-200 bg-white shadow-lg">
                {loadingStations ? (
                  <div className="px-3 py-2 text-sm text-slate-500">Loading stations...</div>
                ) : filteredStations.length === 0 ? (
                  <div className="px-3 py-2 text-sm text-slate-500">No station found</div>
                ) : (
        ...
        <AdminField label="Restro Name">
          <AdminInput value={form.RestroName ?? ""} onChange={(event) => updateField("RestroName", event.target.value)} />
        </AdminField>

        <AdminField label="Brand Name">
          <AdminInput value={form.BrandNameifAny ?? ""} onChange={(event) => updateField("BrandNameifAny", event.target.value)} />
        </AdminField>

        <AdminField label="Raileats Status">
          <AdminSelect value={String(form.RaileatsStatus ?? 0)} onChange={(event) => updateField("RaileatsStatus", Number(event.target.value))}>
        ...
        <AdminField label="IRCTC Approved">
          <AdminSelect value={form.IsIrctcApproved ?? "No"} onChange={(event) => updateField("IsIrctcApproved", event.target.value)}>
            <option>Yes</option>
            <option>No</option>
          </AdminSelect>
        </AdminField>
        ...
        <AdminField label="Restro Rating">
          <AdminInput value={form.RestroRating ?? ""} onChange={(event) => updateField("RestroRating", event.target.value)} />
        </AdminField>

        <AdminField label="Display Photo">
          <AdminInput value={form.RestroDisplayPhoto ?? ""} onChange={(event) => updateField("RestroDisplayPhoto", event.target.value)} />
        </AdminField>
        ...
        <AdminField label="Owner Email">
          <AdminInput value={form.OwnerEmail ?? ""} onChange={(event) => updateField("OwnerEmail", event.target.value)} />
        </AdminField>

        <AdminField label="Owner Phone">
          <AdminInput value={form.OwnerPhone ?? ""} onChange={(event) => updateField("OwnerPhone", event.target.value)} />
        </AdminField>

        <AdminField label="Restro Email">
          <AdminInput value={form.RestroEmail ?? ""} onChange={(event) => updateField("RestroEmail", event.target.value)} />
        </AdminField>

        <AdminField label="Restro Phone">
          <AdminInput value={form.RestroPhone ?? ""} onChange={(event) => updateField("RestroPhone", event.target.value)} />
        </AdminField>
      </div>
