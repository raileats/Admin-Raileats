...
export default function AddressDocsClient({ initialData = {}, imagePrefix = "", states = [], initialDistricts = [] }: Props) {
  ...
  useEffect(() => {
    ...
    function tryPreselectDistrict(list: DistrictItem[]) {
      // debug - remove later if wanted
      // console.log("tryPreselectDistrict: list", list, "initialData.DistrictCode/Name", initialData?.DistrictCode, initialData?.District, initialData?.DistrictName);
      ...
      if (!possibleDistrictCode) {
        return;
      }

      const targetRaw = String(possibleDistrictCode).trim();
      const targetLower = targetRaw.toLowerCase();

      // 1) try exact id/code match
      ...
      // 4) tokenized partial matching: split words and try containing every token
      ...
      // nothing matched — keep empty
      // console.log("tryPreselectDistrict: nothing matched for", possibleDistrictCode);
    }
