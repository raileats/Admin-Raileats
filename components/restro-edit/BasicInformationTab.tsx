...
export default function BasicInformationTab({
  ...
  useEffect(() => {
    if (local?.RestroDisplayPhoto) {
      const img = String(local.RestroDisplayPhoto);
  ...
  async function uploadDisplayPhoto(file: File) {
    ...
    try {
      ...
      if (!["jpg", "jpeg", "png", "webp"].includes(ext)) {
        alert("Only JPG, PNG, JPEG, WEBP image allowed");
        return;
      }
      ...
      alert("Image uploaded. Ab Basic Information ka Save button dabao.");
    } catch (err: any) {
    ...
    <div className="px-4 py-2">
      ...
      <div className="max-w-6xl mx-auto bg-white rounded shadow-sm p-6">
        <FormRow cols={3} gap={6}>
          ...
          <FormField label="Restro Display Photo">
            <input
              type="file"
            ...
            {local?.RestroDisplayPhoto && (
              <div className="mt-1 text-xs text-gray-500 break-all">
                Saved file: {local.RestroDisplayPhoto}
              </div>
            )}
